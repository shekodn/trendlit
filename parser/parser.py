...
def p_assignment(p):
    """assignment : ID snp_checks_for_previous_declaration snp_push_pending_operand snp_update_curr_slice isList EQ snp_push_pending_token expression snp_add_assignation_quad"""

    # ID snp_checks_for_previous_declaration snp_push_pending_operand isList
...
def p_valueSlice2D(p):
    # """valueSlice2D : ID snp_update_curr_slice snp_push_pending_operand OBRACK snp_increase_dim_access_count snp_slice_access_2 snp_push_start_false_bottom slice_expression snp_slice_access2D_3 CBRACK snp_clean_stack_until_false_bottom OBRACK snp_increase_dim_access_count snp_slice_access_2 snp_clean_stack_until_false_bottom slice_expression snp_slice_access2D_final_dim_3 CBRACK snp_clean_stack_until_false_bottom snp_reset_dim_access_count"""
    """valueSlice2D : ID snp_update_curr_slice snp_push_pending_operand OBRACK snp_increase_dim_access_count snp_slice_access_2 CTEI CBRACK snp_slice_access2D_3 OBRACK snp_increase_dim_access_count snp_slice_access_2 CTEI CBRACK snp_slice_access2D_final_dim_3 snp_reset_dim_access_count"""
...
def p_snp_slice_access_2(p):
    ...
    # var_addr = quad_helper.top_operand()
...
def p_snp_slice_access_3(p):
    ...
    if (parser_helper.get_dimensions(slice_name) is 2 and parser_helper.curr_dimension_counter is 1):
        # Add quad: s1*m1
...
def p_snp_slice_access2D_final_dim_3(p):
    ...
    slice_name = parser_helper.curr_slice
    last_token = p[-1]
    # print("LAST_TOKEN", last_token, "slice_name", slice_name)

    if (parser_helper.get_dimensions(slice_name) is 0 or last_token is None):
        return

    if (parser_helper.get_dimensions(slice_name) is 2):
...
def p_snp_update_curr_slice(p):
    ...
    var_name = p[-3]
    if (parser_helper.get_dimensions(var_name) > 0):
